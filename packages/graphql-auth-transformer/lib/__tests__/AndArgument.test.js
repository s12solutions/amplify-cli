...
test('Test "create", "update", "delete" auth operations with "and" parameter forces all rules to pass', () => {
    ...
    expect(out.resolvers['Mutation.updatePost.req.vtl'].replace(/ +?/g, '')).toContain('#if($compoundAuthRuleCounts.testing2==1)\n' +
        '$util.qr($compoundAuthExpressions.testing2.add("#owner0=:identity0"))\n' +
        '#end');
    expect(out.resolvers['Mutation.updatePost.req.vtl'].replace(/ +?/g, '')).toContain('#if($compoundAuthRuleCounts.testing==1)\n' +
    ...
    expect(out.resolvers['Mutation.deletePost.req.vtl'].replace(/ +?/g, '')).toContain('#if($compoundAuthRuleCounts.testing2==1)\n' +
        '$util.qr($compoundAuthExpressions.testing2.add("#owner0=:identity0"))\n' +
        '#end');
    expect(out.resolvers['Mutation.deletePost.req.vtl'].replace(/ +?/g, '')).toContain('#if($compoundAuthRuleCounts.testing==1)\n' +
        '$util.qr($compoundAuthExpressions.testing.add("contains(#groupsAttribute0,:group0$foreach.count)"))\n' +
        '#end');
...
//# sourceMappingURL=AndArgument.test.js.map